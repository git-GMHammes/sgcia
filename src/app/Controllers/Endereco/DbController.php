...
use App\Models\ViewEnderecoCadastroModel;
use Exception;
...
class DbController extends BaseController
{
    ...
    private $ModelViewEnderecoCadastro;
    private $message;
    private $uri;
    private $pagination;
    ...
    public function __construct()
    {
        ...
        $this->ModelViewEnderecoCadastro = new ViewEnderecoCadastroModel();
    ...
    public function dbFields($processRequestFields = array())
    {
        ...
        $dbCreate = array();
        $autoColumn = $this->ModelEndereco->getColumnsFromTable();
    ...
    public function dbFieldsFilter($processRequestFields = array())
    {
        // myPrint($processRequestFields, 'src\app\Controllers\SystemUploadDbController.php', true);
        $dbCreate = array();
        $autoColumn = $this->ModelViewEnderecoCadastro->getColumnsFromTable();
        // myPrint($autoColumn, '', true);
        if (isset($autoColumn['COLUMN'])) {
            foreach ($autoColumn['COLUMN'] as $key_autoColumn => $value_autoColumn) {
                (isset($processRequestFields[$value_autoColumn])) ? ($dbCreate[$value_autoColumn] = $processRequestFields[$value_autoColumn]) : (NULL);
    ...
    public function dbCreate($parameter = NULL)
    {
        ...
        try {
            $this->ModelEndereco->dbCreate($this->dbFields($parameter));
            $affectedRows = $this->ModelEndereco->affectedRows();
            if ($affectedRows > 0) {
                $dbCreate['insertID'] = $this->ModelEndereco->insertID();
    ...
    public function dbUpdate($key, $parameter = NULL)
    {
        if (
            ...
            $this->ModelEndereco->dbUpdate($key, $parameter);
        } else {
            $this->ModelEndereco->dbUpdate($key, $this->dbFields($parameter));
        }
        try {
            $affectedRows = $this->ModelEndereco->affectedRows();
            if ($affectedRows > 0) {
    ...
    {
        try {
            $this->ModelEndereco->dbDelete('id', $parameter);
            $affectedRows = $this->ModelEndereco->affectedRows();
            if ($affectedRows > 0) {
