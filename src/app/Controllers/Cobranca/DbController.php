...
use App\Models\ViewContratosCircuitosCadastroModel;
use App\Models\ViewCobrancaCircuitoCadastroVelocidadeModel;
use Exception;
...
class DbController extends BaseController
{
    ...
    private $ModelViewContratosCircuitosCadastro;
    private $ModelViewCobrancaCircuitoCadastroVelocidadeModel;
    private $message;
    private $uri;
    private $pagination;
    ...
    public function __construct()
    {
        ...
        $this->ModelViewContratosCircuitosCadastro = new ViewContratosCircuitosCadastroModel();
        $this->ModelViewCobrancaCircuitoCadastroVelocidadeModel = new ViewCobrancaCircuitoCadastroVelocidadeModel();
        $this->ModelContato = new CobrancasModel();
    ...
    public function dbFields($processRequestFields = array())
    {
        ...
        $dbCreate = array();
        $autoColumn = $this->ModelContato->getColumnsFromTable();
    ...
    public function dbFieldsFilter($processRequestFields = array())
    {
        // myPrint($processRequestFields, 'src\app\Controllers\SystemUploadDbController.php', true);
        $dbCreate = array();
        $autoColumn = $this->ModelViewContratosCircuitosCadastro->getColumnsFromTable();
        // myPrint($autoColumn, '', true);
        if (isset($autoColumn['COLUMN'])) {
            foreach ($autoColumn['COLUMN'] as $key_autoColumn => $value_autoColumn) {
                (isset($processRequestFields[$value_autoColumn])) ? ($dbCreate[$value_autoColumn] = $processRequestFields[$value_autoColumn]) : (NULL);
    ...
    public function dbCreate($parameter = NULL)
    {
        ...
        try {
            $this->ModelContato->dbCreate($this->dbFields($parameter));
            $affectedRows = $this->ModelContato->affectedRows();
            if ($affectedRows > 0) {
                $dbCreate['insertID'] = $this->ModelContato->insertID();
    ...
    public function dbRead($parameter = NULL, $page = 1, $limit = 10)
    {
        ...
        try {
            if ($parameter !== NULL) {
                $dbResponse = $this
                    ->ModelViewCobrancaCircuitoCadastroVelocidadeModel
                    ->where('id', $parameter)
    ...
    public function dbFilter($parameter = NULL, $page = 1, $limit = 10)
    {
        ...
        try {
            $query = $this
                ->ModelViewContratosCircuitosCadastro
                ->where('deleted_at', NULL);
    ...
    public function dbUpdate($key, $parameter = NULL)
    {
        if (
            ...
            $this->ModelContato->dbUpdate($key, $parameter);
        } else {
            $this->ModelContato->dbUpdate($key, $this->dbFields($parameter));
        }
        try {
            $affectedRows = $this->ModelContato->affectedRows();
            if ($affectedRows > 0) {
    ...
    {
        try {
            $this->ModelContato->dbDelete('id', $parameter);
            $affectedRows = $this->ModelContato->affectedRows();
            if ($affectedRows > 0) {
